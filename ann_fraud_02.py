...
data_train['tipe_kartu'] = data_train.tipe_kartu.astype(str)
data_train['tipe_transaksi'] = data_train.tipe_transaksi.astype(str)
data_train['grouped_time'] = data_train.grouped_time.astype(str)

data_test['tipe_kartu'] = data_test.tipe_kartu.astype(str)
data_test['tipe_transaksi'] = data_test.tipe_transaksi.astype(str)
data_test['grouped_time'] = data_test.grouped_time.astype(str)

# Create dummy variables for some columns
data_train = pd.get_dummies(data_train, columns=['tipe_kartu','tipe_transaksi','grouped_time'], drop_first=True)
data_test = pd.get_dummies(data_test, columns=['tipe_kartu','tipe_transaksi','grouped_time'], drop_first=True)
...
classifier = NN(3, 48, 36, 0.2, 1).ann()
